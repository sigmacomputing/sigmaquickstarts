...
**2:** In the dropdown, select the `BIG_BUYS_OFFICIAL` Cortex Agent. This will route our question directly to the agent instead of Sigma's default Assistant (which is still powered by Cortex)
...
As the agent responds, it provides both top-level analysis and a breakdown of how it built the output.
...
**2:** The first thing that we noticed is that `Southwest` is the lowest performing region out of the whole Big Buys network of stores.
...
**9:** With these 5000+ product reviews, we need to create a quantitative score to leverage in our analysis. We could score them one by one, but let's have Snowflake Cortex run sentiment analysis across all 5,000+ records at once.  
...
**13:** With our new lookup column selected, rename it to `Avg Sentiment` and go to the top bar and hit the decreased decimal point button twice so that we have a simpler number.
...
**1:** From the `Product_Reviews_Big_Buys.csv` table, select the create child element icon and select `Linked input table`:
...
These values should show up as `Null` right now since we haven't entered any changes, but now we're ready to build our model:
...
**11:** Add the `Adjusted profit margin` column to the `X axis` and select `Unstacked` for the bar chart style, but nothing new should show up since we still have not added price changes.
...
**8:** Let's ask the agent to `Review product performance and recommend price changes` by putting that in the chat box and clicking the arrow icon.   
...
**6:** Return to your `Big Buys - {first name_last name}` workbook. Sort the `Price Changes` input table ascending by the `Brand` column, and copy all values from the `Price Change` column:
